...
void CALLBACK olcConsoleGameEngine::waveOutProcWrap(HWAVEOUT hWaveOut, UINT uMsg, DWORD_PTR dwInstance, DWORD_PTR dwParam1, DWORD_PTR dwParam2) {
	((olcConsoleGameEngine*)dwInstance)->olcConsoleGameEngine::waveOutProc(hWaveOut, uMsg, dwParam1, dwParam2);
}
...
float olcConsoleGameEngine::onUserSoundSample(int nChannel, float fGlobalTime, float fTimeStep) {
	return 0.0f;
}

// Overriden by user if they want to manipulate the sound before it is played
float olcConsoleGameEngine::onUserSoundFilter(int nChannel, float fGlobalTime, float fSample) {
	return fSample;
}
...
float olcConsoleGameEngine::GetMixerOutput(int nChannel, float fGlobalTime, float fTimeStep) {
...
std::atomic<bool> olcConsoleGameEngine::m_bAtomActive;
